-- Migration: Add subscription management columns (MySQL Version)
...
-- ALTER TABLE companies ADD COLUMN ban_type VARCHAR(50) DEFAULT 'manual';
...
-- ALTER TABLE blocked_ips ADD COLUMN is_active BOOLEAN DEFAULT TRUE;
...
CREATE INDEX idx_abuse_reports_company_id ON abuse_reports(company_id);
...
-- Insert default subscription plans if not exist (MySQL INSERT IGNORE)
INSERT IGNORE INTO subscription_plans (name, price_monthly, price_yearly, max_employees, features, is_active) 
VALUES 
('Starter', 29.00, 290.00, 10, 'Basic HR features, Up to 10 employees, Email support', 1),
('Professional', 79.00, 790.00, 50, 'Advanced HR features, Up to 50 employees, Priority support, Analytics', 1),
('Enterprise', 199.00, 1990.00, 999999, 'Full HR suite, Unlimited employees, 24/7 support, Custom integrations', 1);
